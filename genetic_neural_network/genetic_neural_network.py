...
class GeneticNeuralNetwork:
    ...
    def __init__(self, geneticSettings):
        ...
        self.slice_sizes = [self.populationSize *
                            x for x in geneticSettings['genetic_operators_size']]
        ...
        self.fitnesses = tf.get_variable("fitnesses", shape=(
            self.populationSize), initializer=tf.zeros_initializer())
    ...
    def run_epoch(self):
        ...
        self.operatorSize = tf.placeholder(
            tf.float32, shape=[len(self.geneticSettings['genetic_operators_size'])])
        ...
        if (self.geneticSettings['fitness'] == 'cross_entropy'):
            ...
            fitness_validation = self.neural_networks.validation_accuracies + \
                self.neural_networks.validation_cost
        ...
        writer = tf.summary.FileWriter(
            self.neural_networks.logdir + '/' + sys.argv[2] + '_' + str(self.populationSize) + '/', sess.graph)
        ...
        for i in range(self.geneticSettings['epochs']):
            ...
            for batch in range((len(train_x)//batch_size) - 1):
                ...
                writer.add_run_metadata(
                    run_metadata, 'step%s' % (str(batch) + '_' + str(i)))
                msg = "Batch: " + str(batch)
                ...
                if (self.fineTuningBoolean):
                    ...
                    for population_slice in self.slice_sizes:
                        slice_finish = int(
                            last_population_slice + population_slice - 1)
                        if (population_slice > 1):
                            operators_max.append(
                                np.mean(cost[last_population_slice:slice_finish]))
                        else:
                    ...
                    slice_with_operator = list(
                        filter(lambda x: x[0] > self.populationSize * 0.05, slice_with_operator))
        ...
        file_string = []
        if (len(sys.argv) > 2):
            file_string = './debug/graphs_logs/' + \
                str(self.populationSize) + '_' + sys.argv[2] + '.pckl'
        else:
            file_string = './debug/graphs_logs/' + \
                str(self.populationSize) + '_sigmoid.pckl'
        with open(file_string, 'wb') as save_graph_file:
            ...
            print('salvei em: ' + './debug/graphs_logs/' +
                  str(self.populationSize) + '.pckl')
