...
from .crossover import generate_child_by_all, generate_child_by_two_points
...
def select_operator_and_apply(genetic_operator, genetic_operator_param, genetic_operator_population_size, elite_size,
    ...
    if (genetic_operator == 'crossover'):
        return crossover_range_operator(genetic_layers, elite_size, genetic_operator_population_size)
            ...
            for idx in range(len(genetic_operators)):
                ...
                genetic_layer.add_operator(
                    *select_operator_and_apply(genetic_operators[idx][0], genetic_operators[idx][1],
                                               tf.cast(
                                                   genetic_operators_size[idx], dtype=tf.int32), elite_size,
...
def crossover_range_operator(genetic_layer: GeneticLayer, tamanhoElite, tamanhoCrossover):
    with tf.name_scope('Crossover'):
        permutations = tf.range(tamanhoCrossover * 2)
        permutations = permutations % tamanhoElite
        # permutations = tf.random_shuffle(permutations)
        ...
        # def add_crossover(permutation):
        #    print(genetic_layer)
        father_tensor = tf.gather(
            genetic_layer.best_weights, permutations[0])
        mother_tensor = tf.gather(
            genetic_layer.best_weights, permutations[1])

        finish_conv = tf.map_fn(lambda x: generate_child_by_two_points(
            father_tensor[0], mother_tensor[1]), permutations, dtype=tf.float32)
        finish_conv = tf.stack(finish_conv)
        # finish_conv = generate_child_by_division(father_tensor, mother_tensor)

        father_tensor = tf.gather(genetic_layer.best_biases, permutations[0])
        mother_tensor = tf.gather(genetic_layer.best_biases, permutations[1])
        # finish_bias = generate_child_by_division(father_tensor, mother_tensor)
        finish_bias = tf.map_fn(lambda x: generate_child_by_two_points(
            father_tensor[0], mother_tensor[1]), permutations, dtype=tf.float32)
        finish_bias = tf.stack(finish_bias)
...
def mutation_operator(genetic_layer: GeneticLayer, tamanhoElite, mutationRate, mutationPercent, tamanhoMutacoes):
    with tf.name_scope('Mutation_operator'):
        ...
        saida_shape = tf.concat(
            [[times_to_repeat], shape_with_ones], axis=0, name="concat")
        tensors_to_mutate = tf.strided_slice(tf.tile(elite_key, saida_shape), [0], [tamanhoMutacoes],
                                             name="stride_tensor_to_mutate")

        finish_conv = mutation(
            tensors_to_mutate, mutationRate, mutationPercent)
    ...
    with tf.name_scope('Mutatio_unbiased'):
        ...
        for key in best_conv:
            ...
            finish_conv[key] = tf.map_fn(
                lambda x: mutation_unbiased(
                    best_conv[key][x % shape_module], mutationRate, mutationPercent),
        ...
        for key in best_bias:
            ...
            finish_bias[key] = tf.map_fn(
                lambda x: mutation_unbiased(
                    best_bias[key][x % shape_module], mutationRate, mutationPercent),
                tf.range(tamanhoMutacoes), dtype=tf.float32)
